...
### 1. Graph vs SQL: які запити складніше написати в SQL?

Найскладнішими для SQL є запити, де потрібно проходити зв'язки на кілька кроків або працювати з поняттям шляху. Наприклад, запит 6 з частини 3 — пошук найкоротшого ланцюжка між двома користувачами через спільні фільми — природно записується в Cypher через `shortestPath`. У графовій моделі ми прямо описуємо шаблон: користувач пов'язаний з фільмом через `RATED`, інший користувач теж пов'язаний з цим або іншим фільмом, і база шукає шлях у графі.

У SQL такий запит швидко стає громіздким, бо треба багато разів виконувати JOIN таблиці `ratings` до самої себе. Для шляху довжини 2 це ще просто:

```sql
SELECT r1.user_id AS user1,
       r2.user_id AS user2,
       r1.movie_id
FROM ratings r1
...
Але для шляху довжини 4 вже потрібен проміжний користувач і два спільні фільми:

```sql
SELECT r1.user_id AS source_user,
       r2.user_id AS middle_user,
       r4.user_id AS target_user,
       r1.movie_id AS first_shared_movie,
       r3.movie_id AS second_shared_movie
FROM ratings r1
JOIN ratings r2 ON r1.movie_id = r2.movie_id
...
WHERE r1.user_id = 1
  ...
  AND r2.user_id <> r4.user_id;
```

Це лише один фіксований варіант довжини шляху. Якщо треба знайти найкоротший шлях невідомої довжини, SQL-запит доведеться писати через рекурсивні CTE, контролювати цикли, обмежувати глибину й вручну обирати найкоротший результат. У Cypher це значно природніше, бо шлях є базовим поняттям мови. Так само рекомендаційний запит із частини 3 простіше читати як графовий обхід `User -> Movie <- SimilarUser -> RecommendedMovie`, ніж як кілька JOIN'ів таблиці оцінок.

### 2. Де граф програє реляційній моделі?

Графова модель не завжди краща. Для простих масових агрегацій по всьому датасету реляційна модель часто зручніша й передбачуваніша. Наприклад, якщо потрібно побудувати звіт із середнім рейтингом за роками, кількістю оцінок по кожній віковій групі або експортувати всі оцінки у табличному вигляді, SQL буде простішим. Такі задачі не потребують складних обходів графа; вони працюють із великими таблицями, групуванням, фільтрами й агрегатами.

Наприклад, запит «середній рейтинг по жанрах» можна виконати і в графі, але в SQL це теж природна операція `GROUP BY`. Якщо жанри зберігати в нормалізованій таблиці `movie_genres`, SQL-запит буде прямим і зрозумілим:
...
Також реляційна модель краще підходить для перевірки обмежень цілісності, масового експорту та інтеграції з інструментами, які очікують таблиці. Граф виграє тоді, коли важлива структура зв'язків, але може бути менш зручним або повільнішим, коли задача зводиться до табличної статистики.

### 3. Покращення схеми для прискорення запитів

Перше покращення — зберігати попередньо обчислені агрегати на вузлі `Movie`, наприклад `ratingsCount` і `averageRating`. Це прискорило б запит 1 з частини 3, де ми шукаємо фільми жанру `Thriller` із середнім рейтингом вище `4.0`. Зараз середня оцінка рахується через обхід усіх ребер `RATED` для кожного фільму. Якщо ці значення періодично оновлювати під час завантаження або окремим batch-скриптом, запит міг би одразу фільтрувати за властивістю `m.averageRating`, не перераховуючи агрегацію щоразу.

Друге покращення — матеріалізувати схожість користувачів у ребрах `SIMILAR` для рекомендацій. Запит 5 кожного разу шукає схожих користувачів через спільні високо оцінені фільми. Це дорого, бо потребує багатьох обходів шаблону `User -> Movie <- User`. Якщо заздалегідь створити ребра `(:User)-[:SIMILAR {weight}]->(:User)` для пар із достатньою кількістю спільних улюблених фільмів, рекомендаційний запит стане коротшим: спочатку перейти до схожих користувачів, а потім до їхніх високо оцінених фільмів.

Третє можливе покращення — додати окремий тип ребра для позитивних оцінок, наприклад `LIKED`, який створюється тільки для `rating >= 4` або `rating = 5`. Багато запитів у роботі використовують саме високі оцінки: рекомендації, PageRank через спільні високі оцінки, Louvain через спільні п'ятизіркові фільми. Якщо такі зв'язки винести в окремий тип, запити можуть обходити менше ребер і не перевіряти умову `r.rating >= 4` на кожному кроці. Недолік — дублювання частини інформації, тому таке рішення варто застосовувати тільки для запитів, які виконуються часто.
